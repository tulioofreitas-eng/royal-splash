...
test.describe("Reforma-RJ Structured Intake Form", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/lp/reforma-rj");
    // Wait for form to be present
    await expect(page.locator("[data-reforma-intake-form]")).toBeVisible();
  });

  // Rendering & Field Collection
  test("1: form renders with all required fields", async ({ page }) => {
    const form = page.locator("[data-reforma-intake-form]");
  ...
  // Direct WhatsApp (regression)
  test("8: direct WhatsApp button exists on page", async ({ page }) => {
    // Look for WhatsApp link in page
    const waLinks = await page.locator('a[href*="wa.me"]').count();
    expect(waLinks).toBeGreaterThan(0);
  });

  // Protocol line
  test("9: protocol line element exists in success state", async ({ page }) => {
  ...
  // Responsive design
  test("12: mobile viewport (390px) renders", async ({ page }) => {
  ...
  test("13: tablet viewport (768px) renders", async ({ page }) => {
  ...
  test("14: desktop viewport (1440px) renders", async ({ page }) => {
  ...
  // Runtime
  test("15: no console errors on interaction", async ({ page }) => {
    const errors: string[] = [];
    page.on("console", (msg) => {
      if (msg.type() === "error") {
  ...
  test("16: form submission structure is valid", async ({ page }) => {
    const form = page.locator("[data-reforma-intake-form]");
  ...
  test("17: optional fields can be left empty", async ({ page }) => {
    const form = page.locator("[data-reforma-intake-form]");
  ...
  test("18: form uses data attributes correctly", async ({ page }) => {
    expect(await page.locator("[data-reforma-intake-form]").count()).toBeGreaterThan(0);
  ...
  test("19: submit button text is visible", async ({ page }) => {
    const form = page.locator("[data-reforma-intake-form]");
    const submitBtn = form.locator('button[type="submit"]');
  ...
  test("20: character counter for necessidade works", async ({ page }) => {
    const form = page.locator("[data-reforma-intake-form]");
    const necessidade = form.locator('textarea[name="necessidade"]');
    const counter = form.locator('#reforma-necessidade-count');
