...
for (const viewport of viewports) {
  test(`Lazer media-rich Brand landing at ${viewport.width}px`, async ({ page }) => {
    await page.setViewportSize(viewport);
    const response = await page.goto("/lp/lazer");
    expect(response?.ok()).toBe(true);
    ...
    const h1 = page.getByRole("heading", { level: 1, name: "Sua área de lazer completa, do projeto à entrega" });
    await expect(h1).toHaveCSS("font-family", /Cormorant Garamond/);
    await expect(page.getByRole("heading", { level: 2, name: "Excelência em cada detalhe" })).toHaveCSS("font-family", /Cormorant Garamond/);
    const headings = await page.locator("main :is(h1, h2, h3)").evaluateAll((nodes) => nodes.map((node) => Number(node.tagName.slice(1))));
    expect(headings[0]).toBe(1);
    for (let index = 1; index < headings.length; index += 1) expect(headings[index] - headings[index - 1]).toBeLessThanOrEqual(1);
    ...
    const whatsappBox = await whatsapp.boundingBox();
    expect(whatsappBox).not.toBeNull();
    for (const element of await page.locator("main#main-content :is(h1, h2, h3, p, a, summary)").all()) {
      if (!(await element.isVisible())) continue;
      const contentBox = await element.boundingBox();
      if (contentBox && whatsappBox) expect(overlaps(contentBox, whatsappBox), `WhatsApp overlaps readable content: ${await element.innerText()}`).toBe(false);
    }
    ...
    const heroImage = page.getByRole("img", { name: "Área de lazer de alto padrão" });
    await expect(heroImage).toHaveAttribute("srcset", /\s390w(?:,|$)/);
    await expect(heroImage).toHaveAttribute("srcset", /\s1792w(?:,|$)/);
    await expect(heroImage).toHaveAttribute("sizes", "100vw");

    const expectedAlts = ["Área de lazer completa", "Área gourmet integrada", "Deck e paisagismo", "Área de lazer à noite"];
    const galleryButtons = page.locator(".galeria-zoom-item");
    await expect(galleryButtons).toHaveCount(4);
    for (let index = 0; index < expectedAlts.length; index += 1) {
      const thumbnail = galleryButtons.nth(index).getByRole("img", { name: expectedAlts[index] });
    ...
    const expectedLightboxSrc = await firstGalleryButton.getAttribute("data-lightbox-src");
    ...
    const disclosures = page.locator(".lazer-faq details");
    await expect(disclosures).toHaveCount(4);
    for (const question of faqQuestions) await expect(page.locator(".lazer-faq summary", { hasText: question })).toBeVisible();
    ...
    await expect(firstDisclosure.getByText("Piscina, deck, área gourmet, paisagismo e iluminação são planejados como um único projeto integrado — você recebe a visualização completa antes de aprovar.", { exact: true })).toBeVisible();
    await firstDisclosure.locator("summary").click();
    ...
    await page.locator("astro-dev-toolbar").evaluateAll((nodes) => nodes.forEach((node) => node.remove()));
    await page.evaluate(() => { if (document.activeElement instanceof HTMLElement) document.activeElement.blur(); window.scrollTo(0, 0); });
    await mkdir("/tmp/royal-p2f-wp3b", { recursive: true });
    await page.screenshot({ path: `/tmp/royal-p2f-wp3b/lazer-${viewport.width}.png`, fullPage: true });
  });
}
